...
interface ClientFormProps {
  ...
  onClose: () => void;
  onSave: (data: Omit<Client, "id" | "createdAt" | "updatedAt">) => Promise<void>;
  title: string;
}
...
function ClientForm({ initial, onClose, onSave, title }: ClientFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await onSave({ name: name.trim(), email: email.trim() || undefined, phone: phone.trim() || undefined, address: address.trim() || undefined, notes: notes.trim() || undefined, tags: tags.length ? tags : undefined, ownerId: user.id });
      onClose();
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Nombre del cliente" required autoFocus
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Correo</label>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="correo@ejemplo.com"
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-white dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Teléfono</label>
          ...
          <div>
            ...
            <input type="text" value={address} onChange={e => setAddress(e.target.value)} placeholder="Dirección o ciudad"
              className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-white dark:bg-slate-800 dark:border-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Notas</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notas internas..." rows={3}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Etiquetas</label>
            <div className="flex gap-2 mb-2 flex-wrap">
              {tags.map(t => (
                <span key={t} className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400 text-xs font-medium">
                  #{t}
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={addTag} className="px-3 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-slate-700 dark:hover:bg-slate-600 rounded-xl text-sm font-medium transition-colors">Añadir</button>
            </div>
          </div>
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-800 transition-colors">Cancelar</button>
            <button type="submit" disabled={saving || !name.trim()} className="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl transition-colors disabled:opacity-50">
...
export default function ClientsPage() {
  ...
  return (
    <>
              ...
              <motion.div key={client.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-gray-900 dark:text-slate-100 truncate">{client.name}</h3>
                    <div className="flex items-center gap-3 mt-0.5 flex-wrap">
                      {client.email && (
                        <a href={`mailto:${client.email}`} className="flex items-center gap-1 text-xs text-gray-500 dark:text-slate-400 hover:text-blue-600 transition-colors">
                          <Mail size={11} />{client.email}
                        </a>
                      )}
                      {client.phone && (
                        <a href={`tel:${client.phone}`} className="flex items-center gap-1 text-xs text-gray-500 dark:text-slate-400 hover:text-green-600 transition-colors">
                    ...
                    {client.tags && client.tags.length > 0 && (
                      <div className="flex gap-1.5 mt-1.5 flex-wrap">
                        {client.tags.map(t => (
                          <span key={t} className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400 text-[10px] font-medium">#{t}</span>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <button onClick={() => setExpanded(expanded === client.id ? null : client.id)}
                      ...
                      <ChevronDown size={16} className={cn("transition-transform", expanded === client.id && "rotate-180")} />
                    </button>
                  ...
                  {expanded === client.id && client.notes && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-sm text-gray-600 dark:text-slate-400 whitespace-pre-wrap">{client.notes}</p>
                      </div>
                      <p className="text-[10px] text-gray-400 dark:text-slate-500 mt-2">
                        Creado {format(parseISO(client.createdAt), "d MMM yyyy", { locale: es })}
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
      ...
      <AnimatePresence>
        {showCreate && (
          <ClientForm title="Nuevo Cliente" onClose={() => setShowCreate(false)} onSave={handleCreate} />
        )}
        {editing && (
          <ClientForm title="Editar Cliente" initial={editing} onClose={() => setEditing(null)} onSave={handleUpdate} />
        )}
      </AnimatePresence>
    </>
  );
